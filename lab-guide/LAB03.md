...
```Console
cd ~/environment/moving-to-serverless-immersion-day/LAB03/
```
...
New python executable in /home/ec2-user/environment/moving-to-serverless-immersion-day/LAB03/venv/bin/python36
Also creating executable in /home/ec2-user/environment/moving-to-serverless-immersion-day/LAB03/venv/bin/python
Installing setuptools, pip, wheel...done.
```

2. Now you can activate your **virtualenv** (`venv`).
```Console
source ~/environment/moving-to-serverless-immersion-day/LAB03/venv/bin/activate
echo "unalias python" >> ~/.bash_profile
echo "source ~/environment/moving-to-serverless-immersion-day/LAB03/venv/bin/activate" >> ~/.bash_profile
```
...
* output :
```
~/environment/moving-to-serverless-immersion-day/LAB03/venv/bin/python
```
...
* output:
```
~/environment/moving-to-serverless-immersion-day/LAB03/venv/bin/pip
```
...
```Console
pip install -r ~/environment/moving-to-serverless-immersion-day/LAB03/02-CloudAlbum-Chalice/cloudalbum/requirements.txt
```
...
```console
mkdir -p ~/environment/moving-to-serverless-immersion-day/LAB03/01-Chalice/
```
```console
cd ~/environment/moving-to-serverless-immersion-day/LAB03/01-Chalice/
```
```console
...
* output: 
```
<username>:~/environment/moving-to-serverless-immersion-day/LAB03/01-Chalice (master) $ tree -a .
.
└── myapp
    ├── app.py
    ├── .chalice
...
* Replace `app.py` file with the contents of above source code.
  * `app.py` file is located in `~/environment/moving-to-serverless-immersion-day/LAB03/01-Chalice/myapp/app.py`
...
11. Deploy to **API Gateway** and **Lambda**. You can deploy this application using Chalice CLI command. (Make sure, you working directory is `~/environment/moving-to-serverless-immersion-day/LAB03/01-Chalice/myapp`)
```console
chalice deploy
```
* output
...
* You can check the files of `deployed` and `deployments` directories. (~/environment/moving-to-serverless-immersion-day/LAB03/01-Chalice)
```
tree -a .
```
* output:
...
```console
cd ~/environment/moving-to-serverless-immersion-day/LAB03/01-Chalice/myapp
```
```console
chalice delete
```
...
15. Let's take a look around `~/environment/moving-to-serverless-immersion-day/LAB03/02-CloudAlbum-Chalice/cloudalbum/` directory.

```console
cd ~/environment/moving-to-serverless-immersion-day/LAB03/02-CloudAlbum-Chalice/cloudalbum/
```
```console
tree -L 2 -a .
```
...
```console
aws s3 sync ~/environment/moving-to-serverless-immersion-day/resources/static s3://cloudalbum-<INITIAL>/static/ --acl public-read
```
...
23. Review template files. Template files which stored `moving-to-serverless-immersion-day/LAB03/02-CloudAlbum-Chalice/cloudalbum/chalicelib/templates` are already changed to **load static resources in your S3 bucket**. You can refer above variable in the config.py file.
```python
S3_STATIC_URL = "https://s3-{0}.amazonaws.com/{1}/static".format(conf['AWS_REGION'], conf['S3_PHOTO_BUCKET'])
```
